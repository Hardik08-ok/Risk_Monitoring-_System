...
// ═══════════════════════════════════════════════════════════════════════════════
// LiveDisastersPage
// A full-page live disasters map mode. Fetches from /api/disasters every 10 min.
// ═══════════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, height: 'calc(100vh - 88px)' }}>
      ...
      {someFeedsFailed && !loading && (
        <div style={{
          background: 'rgba(234,88,12,0.12)', border: '1px solid #ea580c',
          borderRadius: 8, padding: '8px 14px', color: '#fdba74', fontSize: 11,
          flexShrink: 0,
        }}>
          ⚠️ Partial data: {sourceErrors.map(e => e.source).join(', ')} feed unavailable. Showing events from remaining sources.
        </div>
      )}
      {error && (
        <div style={{
          background: 'rgba(220,38,38,0.15)', border: '1px solid #dc2626',
          borderRadius: 8, padding: '8px 14px', color: '#fca5a5', fontSize: 11,
          flexShrink: 0,
        }}>
          ⚠️ {error}
        </div>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 300px', gap: 12, minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, overflow: 'auto', minHeight: 0 }}>
              ...
              {Object.entries(DISASTER_TYPE_META).map(([type, meta]) => (
                <div key={type} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span style={{
                    ...
                    color: typeCounts[type] ? '#f1f5f9' : '#374151',
                  }}>
                    {typeCounts[type] ?? 0}
                  </span>
                </div>
          ...
          {!loading && events.length === 0 && (
            <div style={{
              background: '#1e293b', borderRadius: 8, padding: '20px 12px',
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>
            {events.slice(0, 40).map(ev => (
              <DisasterEventCard key={ev.id} ev={ev} />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="slide-in" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: '#f1f5f9', flex: 1, lineHeight: 1.3 }}
              title={ev.title}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        ...
        {ev.magnitude != null && (
          <span style={{ fontSize: 9, color: '#fbbf24' }}>M{ev.magnitude.toFixed(1)}</span>
        )}
        {dateStr && (
          <span style={{ fontSize: 9, color: '#475569', marginLeft: 'auto' }}>{dateStr}</span>
        )}
      </div>
      {ev.link && (
        <a href={ev.link} target="_blank" rel="noreferrer"
           style={{ fontSize: 9, color: '#60a5fa', display: 'block', marginTop: 3 }}>
          View source ↗
        </a>
      )}
    </div>
